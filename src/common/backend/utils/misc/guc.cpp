...
static bool check_timezone_abbreviations(char** newval, void** extra, GucSource source)
{
    ...
    /* Try to load the file; if it fails, just return true anyway.
     * pg_timezone_abbrev_initialize() will retry later when PGSHAREDIR is set. */
    *extra = load_tzoffsets(*newval);
    if (!*extra) {
        ereport(WARNING, (errmsg("could not load timezone abbreviations \"%s\", will retry later", *newval)));
        return true;
    }
